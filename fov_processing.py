...
def calculate_fovs(width, height, distance):
    ...
    logging.debug(f'Calculated FOVs - H: {h_fov}, V: {v_fov}, D: {d_fov}')
...
def process_measurements(listbox, chart_169_width, chart_169_height, chart_43_width, chart_43_height):
    ...
    logging.debug(f'Measurements retrieved from listbox: {measurements}')
    ...
    for measurement in measurements:
        ...
        logging.debug(f'Processing measurement: {parts}')
    ...
    logging.debug(f'DataFrame created and sorted by Device: {df}')
    ...
    try:
        with pd.ExcelWriter("FOV_Measurements.xlsx", engine='openpyxl') as writer:
            ...
            logging.debug('FOV Data written to Excel')
            ...
            logging.debug('Chart Size Info written to Excel')
        ...
        logging.info('Excel file "FOV_Measurements.xlsx" has been created successfully with auto-spacing.')
        ...
        logging.error('Permission denied: Excel file might be open')
